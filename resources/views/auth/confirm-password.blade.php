{{--
  Zone sécurisée — confirmation du mot de passe.

  Huitième écran, hors des sept maquettes, mais posé sur le même gabarit :
  aucune page d'authentification ne doit détonner.
--}}
<x-auth-layout
    :title="__('auth.confirm.titre')"
    aside-tone="dark"
    :aside-title="__('auth.confirm.aside_titre')"
    :aside-text="__('auth.confirm.aside_texte')"
    :aside-step="2">

    {{-- Écran hors des maquettes fournies : visuel dérivé de la connexion. --}}
    <x-slot:aside>
        <x-visual.profile-card :cta="false" />
        <x-visual.chip icon="bouclier" :label="__('auth.confirm.visuel_zone')" position="bas-droite" />
    </x-slot:aside>

    <h1 class="auth__title">{{ __('auth.confirm.titre') }}</h1>
    <p class="auth__lead">{{ __('auth.confirm.accroche') }}</p>

    <form method="POST" action="{{ route('password.confirm.store') }}" novalidate class="mt-4">
        @csrf

        <div class="auth-fields">
            <x-auth-password name="password" autocomplete="current-password" autofocus />

            <x-button :block="true">{{ __('auth.confirm.bouton') }}</x-button>
        </div>
    </form>

    <p class="f__hint text-center mt-4 mb-0">
        <a href="{{ route('dashboard') }}">{{ __('auth.liens.retour_espace') }}</a>
    </p>
</x-auth-layout>
