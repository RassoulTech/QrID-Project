...
<div class="dropdown">
    ...
    <div class="dropdown-menu dropdown-menu-end notif-menu" aria-labelledby="menuNotifications">
        <div class="notif-menu__tete">
            <span class="notif-menu__titre">Notifications</span>

            @if ($nonLues > 0)
                <form method="POST" action="{{ route('notifications.read-all') }}">
                    @csrf
                    <button type="submit" class="notif-menu__tout">Tout marquer comme lu</button>
                </form>
            @endif
        </div>
        ...
        @empty
            ...
            <div class="notif-vide">
                ...
                <span class="notif-vide__aide">
                    Vous serez prévenu dès la première consultation de votre carte.
                </span>
            </div>
        @endforelse

        @if ($alertes->isNotEmpty())
            <a class="notif-menu__pied" href="{{ route('notifications.index') }}">Voir tout</a>
        @endif
    </div>
</div>
