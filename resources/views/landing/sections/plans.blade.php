...
<section class="section section--tint" id="tarifs">
  <div class="wrap">
    <h2 class="section-title">{!! __('Tarifs simples &amp; transparents') !!}</h2>
    <p class="section-sub">{{ __('Payable via Wave, Orange Money et Free Money.') }}</p>
    ...
    <div class="plans">
      @foreach ($plans as $plan)
        @php
            ...
            $cta = $plan->isFree() ? __('Essayer gratuitement') : __("S'abonner");
        @endphp

        <div class="plan{{ $featured ? ' plan--featured' : '' }}" data-reveal>
          @if ($featured)
            <span class="plan__badge">{{ __('Best value') }}</span>
          @endif

          <div class="plan__name">{{ __($plan->name) }}</div>

          <div class="plan__price">
            {{ number_format($plan->price_fcfa, 0, ',', ' ') }}<small> FCFA</small>
          </div>

          <div class="plan__period">{{ __($period) }}</div>
