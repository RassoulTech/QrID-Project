...
<x-admin-layout
    ...
    subtitle="Gérer les offres tarifaires proposées aux clients."
>
    <nav class="adm-onglets" aria-label="Sections des paramètres">
        @foreach ($onglets as $cle => $libelle)
            <a href="{{ route('admin.settings', ['onglet' => $cle === 'plans' ? null : $cle]) }}"
               @class(['adm-onglet', 'is-active' => $onglet === $cle])
               @if ($onglet === $cle) aria-current="true" @endif>{{ $libelle }}</a>
        @endforeach
    </nav>
    ...
    @if ($onglet !== 'plans')
        ...
        <div class="adm-bloc">
            <x-empty-state icon="document"
                title="Aucun réglage modifiable ici"
            ...
            <div class="adm-grille adm-grille--1-1" style="margin-bottom:0">
                ...
                <section class="adm-bloc">
                    <div class="adm-bloc__tete">
                        <h2 class="adm-bloc__titre">Offres actuelles</h2>
                    </div>
                        ...
                        <a href="{{ route('admin.settings.plan', $plan) }}"
                           @class(['adm-offre', 'is-active' => $selection && $selection->is($plan)])>
                            <span class="adm-offre__tete">
                                ...
                                @if (! $plan->is_active)
                                    <x-badge variant="secondary">Retirée</x-badge>
                                @endif
                            </span>
                    ...
                    @empty
                        <x-empty-state icon="payment"
                            ...
                            message="Créez une première formule pour ouvrir la vente." />
                    @endforelse
                        ...
                        <form method="POST" action="{{ route('admin.settings.plan.store') }}" class="adm-form">
                            ...
                            <div class="adm-champ">
                                ...
                                <span class="adm-champ__aide">
                                    Définitif : il est inscrit dans chaque paiement et ne pourra plus être modifié.
                                </span>
                                @error('slug')<span class="adm-modale__erreur">{{ $message }}</span>@enderror
                            </div>

                            <div class="adm-champ">
                                <label for="new_price">Prix en FCFA</label>
                                <input type="number" id="new_price" name="price_fcfa" class="adm-modale__champ"
                                       value="{{ old('price_fcfa', 0) }}" required min="0" step="1">
                                <span class="adm-champ__aide">
                                    Nombre entier : le franc CFA n'a pas de subdivision en circulation.
                                </span>
                            </div>

                            <div class="adm-champ">
                                <label for="new_duration">Périodicité</label>
                            ...
                            <button type="submit" class="adm-btn adm-btn--vert">Créer la formule</button>
                        </form>
                    </details>
                </section>
                ...
                <section class="adm-bloc">
                    @if ($selection === null)
                        <x-empty-state icon="payment"
                            title="Aucune formule à modifier"
                            message="Créez d'abord une formule dans la colonne de gauche." />
                    @else
                        <div class="adm-bloc__tete">
                            <h2 class="adm-bloc__titre">Éditeur de formule</h2>
                        ...
                        <form method="POST" action="{{ route('admin.settings.plan.update', $selection) }}"
                            ...
                            <div class="adm-champ">
                                <label for="slug_lecture">Identifiant technique</label>
                                ...
                                <span class="adm-champ__aide">
                                    Non modifiable : cet identifiant est inscrit dans
                                    chaque paiement déjà encaissé.
                                </span>
                            </div>

                            <div class="adm-champ">
                                <label for="price_fcfa">Prix en FCFA</label>
                                <input type="number" id="price_fcfa" name="price_fcfa" class="adm-modale__champ"
                                       value="{{ old('price_fcfa', $selection->price_fcfa) }}"
                                       required min="0" step="1">
                                <span class="adm-champ__aide">
                                    Les abonnements en cours ne sont pas touchés : le
                                    nouveau tarif s'applique aux souscriptions suivantes.
                                </span>
                                @error('price_fcfa')<span class="adm-modale__erreur">{{ $message }}</span>@enderror
                            </div>

                            <div class="adm-champ">
                                <label for="duration_days">Périodicité</label>
                            ...
                            <fieldset class="adm-champ">
                                ...
                                <input type="text" name="features[]" class="adm-modale__champ adm-champ__ligne"
                                       maxlength="160" placeholder="Ajouter un élément…"
                                       aria-label="Nouvel élément inclus">
                                <input type="text" name="features[]" class="adm-modale__champ adm-champ__ligne"
                                       maxlength="160" placeholder="Ajouter un élément…"
                                       aria-label="Nouvel élément inclus">

                                <span class="adm-champ__aide">
                                    Videz une ligne pour retirer l'élément correspondant.
                                </span>
                            </fieldset>
                            ...
                            <label class="adm-coche">
                                ...
                                <span>Formule proposée à la vente</span>
                            </label>

                            <button type="submit" class="adm-btn adm-btn--vert">Enregistrer</button>
                        </form>
                    @endif
                </section>
            </div>
